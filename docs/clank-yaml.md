...
flow) projects. Dependencies are installed by the project's own
package manager, resolved in this order:

1. `package.json` `packageManager` (the corepack convention, e.g.
   `"pnpm@9.1.0"`) or `devEngines.packageManager` — authoritative.
2. The lockfile: `bun.lock`/`bun.lockb` → bun, `pnpm-lock.yaml` →
...
3. No signal → bun (clank-created templates ship no lockfile; bun
