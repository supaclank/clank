...
| `question` | `{ request_id, tool_use_id?, questions[] }` | Agent asks structured question(s); normalized across backends (Claude `AskUserQuestion`, OpenCode `question`). Each question: `{ text, header?, multi_select?, allow_custom?, options[{label, description?}] }`. Reply via `POST /sessions/{id}/questions/{request_id}/reply`. Supersedes the `permission` event with the **same `request_id`** (emitted for legacy clients). Still-pending questions are **replayed into every new SSE subscription** ([QST-004](11-interactive-tools.md)) — apply idempotently by `request_id`. | [11](11-interactive-tools.md#the-question-event-normalized-path) |
| `question.resolved` | `{ request_id }` | A pending question was answered or dismissed (possibly on another client) — clear the card. | [11](11-interactive-tools.md#the-question-event-normalized-path) |
