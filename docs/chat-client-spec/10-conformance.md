...
| `CONF-PENDING-PERM-GAP` | a (re)join to a blocked session recovers the parked prompt from `/pending-permission` (and the parked interactive tool part from history); an empty queue after a restart is surfaced honestly (not shown as plain "working") | OP-007, INV-PENDING-PERM-GAP-001 |
