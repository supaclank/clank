...
// TestSettingsView_ActiveHubShowsOverride pins the override-aware
// rendering: when --hub-url was passed at startup, the row must say
// so visibly (not silently render whatever prefs.json contains),
// because the override pins the live connection regardless of the
// pref toggle.
func TestSettingsView_ActiveHubShowsOverride(t *testing.T) {
	t.Parallel()

	// User has prefs saying "remote → http://configured" but launched
	// with --hub-url http://override:9999. The row should reflect the
	// override, not the pref.
	s := newSettingsView("", "", "remote", "http://configured", "http://override:9999")
	var got string
	for _, e := range s.entries {
		if e.kind == settingsEntryActiveHub {
			got = e.value
	...
	if !strings.Contains(got, "http://override:9999") {
		t.Errorf("override should be reflected in value, got %q", got)
	}
	if !strings.Contains(got, "overridden") {
		t.Errorf("override should be labeled, got %q", got)
	}
	if strings.Contains(got, "http://configured") {
		t.Errorf("pref URL should NOT leak when override is active, got %q", got)
	}
}
