...
// questionBlockMessages is the stream sequence for an AskUserQuestion tool
// block auto-running in bypass mode (start → input delta → stop).
func questionBlockMessages(sessionID, toolUseID string) []claudecode.Message {
	return []claudecode.Message{
		...
		&claudecode.StreamEvent{
			...
			Event: map[string]any{
				...
				"delta": map[string]any{
					"type":         "input_json_delta",
					"partial_json": `{"questions":[{"question":"Q?","header":"Q","options":[{"label":"A"}]}]}`,
				},
			},
		},
		&claudecode.StreamEvent{
...
// A bypass question the model moved past (a later assistant content block
// arrived) must auto-resolve, so a client that connects later never gets a
// stale card. Regression for the pending-question replay path.
func TestClaudeCodeBackend_Question_BypassResolvedWhenConversationMovesOn(t *testing.T) {
	t.Parallel()
	const sessionID = "claude-q-movedon"
	msgs := questionBlockMessages(sessionID, "toolu_stale")
	// The model continues: a new message cycle with a text block.
	msgs = append(msgs,
		&claudecode.StreamEvent{
			SessionID: sessionID,
			Event: map[string]any{
				"type":    "message_start",
				"message": map[string]any{"id": "msg_2"},
			},
		},
		&claudecode.StreamEvent{
			SessionID: sessionID,
			Event: map[string]any{
				...
				"content_block": map[string]any{
					"type": "text",
					"text": "Proceeding with JWT since you didn't answer.",
				},
			},
		},
	)
	...
	if err := b.RespondQuestion(context.Background(), qData.RequestID, nil, true); err == nil {
		t.Error("RespondQuestion after moved-on resolution = nil, want unknown-id error")
	}
}

// A user follow-up supersedes a pending bypass question: the model already
// proceeded without the answer, so a later send must retire the prompt.
func TestClaudeCodeBackend_Question_BypassResolvedOnUserSend(t *testing.T) {
	t.Parallel()
	const sessionID = "claude-q-usersend"
	transport := newMockTransport(questionBlockMessages(sessionID, "toolu_send"))
	b := agent.NewClaudeCodeBackend(t.TempDir())
	defer b.Stop()
	_ = captureOpenOptions(t, b, transport)
	...
	if err := b.Send(context.Background(), agent.SendMessageOpts{Text: "actually, do it your way"}); err != nil {
		t.Fatalf("Send: %v", err)
	}
	rEvt := waitForEventType(t, b.Events(), agent.EventQuestionResolved, 2*time.Second)
	if rEvt.Data.(agent.QuestionResolvedData).RequestID != qData.RequestID {
