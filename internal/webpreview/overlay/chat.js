...
// 400s on missing keys and never fills values in; the id contract is
// internal/agent/presets BuiltinDefaultPrefix). null when absent: the
...
export const defaultPresetConfig = (presetList, backend) => {
  ...
  const p = presetList.find((x) => x && x.backend === backend && x.id === 'builtin-default-' + backend);
