...
func TestBootstrapShell_PreservesForeignNodeModules(t *testing.T) {
	...
	if err := runBootstrap(t, shellCmd, workDir, marker, "npm install"); err != nil {
	...
	if err := runBootstrap(t, shellCmd, workDir, marker, "npm install", bootstrapWipeEnv+"=1"); err != nil {
...
func TestNeedsNodeModulesWipe(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "switch to a freeform install command: user owns the tree",
			completed: "bun",
			installer: "pnpm install --frozen-lockfile",
			want:      false,
		},
		{
			name:      "switch from a freeform install command: no proven contamination",
			completed: "make deps",
			installer: "bun",
			want:      false,
		},
