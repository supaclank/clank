...
func TestShellQuoteJoin(t *testing.T) {
	...
	cases := []struct {
		...
		{"plain args pass through", []string{"bun", "expo", "run:android", "--device"}, "bun expo run:android --device"},
...
func TestFixCmd_ChildFlagsStayInArgs(t *testing.T) {
	...
	cmd.SetArgs([]string{"bun", "expo", "run:android", "--device"})
	if err := cmd.Execute(); err != nil {
		t.Fatalf("Execute: %v", err)
	}
	want := []string{"bun", "expo", "run:android", "--device"}
...
func TestRunFix_CreatesSessionWithFixPrompt(t *testing.T) {
	...
	if got := last.LastSendOpts().Text; !strings.Contains(got, "<command>bun expo run:android</command>") {
...
func TestFixPrompt_ContainsCommandLine(t *testing.T) {
	...
	prompt := fixPrompt([]string{"bun", "expo", "run:android"})
	if !strings.Contains(prompt, "<command>bun expo run:android</command>") {
