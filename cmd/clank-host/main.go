...
func run(cfg runConfig) error {
	...
	for _, bt := range acpSet {
		...
		case agent.BackendHermes:
			hermesMgr, err := host.NewHermesACPManager()
			if err != nil {
				return fmt.Errorf("--acp-backends: hermes: %w", err)
			}
			backendManagers[agent.BackendHermes] = hermesMgr
		case agent.BackendPi:
			piMgr, err := host.NewPiACPManager(acpDirs)
			if err != nil {
				return fmt.Errorf("--acp-backends: pi: %w", err)
			}
			backendManagers[agent.BackendPi] = piMgr
		default:
